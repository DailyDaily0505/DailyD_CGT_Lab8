...
void CreateDuck(PhysicsShapeList<PhysicsSprite>& duckList, bool drawingArrow, PhysicsSprite arrow, World world, PhysicsShapeList<PhysicsSprite> ducks, PhysicsRectangle right, int score, Texture redTex)
{
    PhysicsSprite& duck = ducks.Create();
    duck.setTexture(redTex);
    Vector2f sz = duck.getSize();
    duck.setCenter(Vector2f(-20 , 20 + (sz.y / 2)));
...
int main()
{
    ...
    right.setCenter(Vector2f(900, 300));
    right.setStatic(true);
    world.AddPhysicsBody(right);

    Texture redTex;
    LoadTex(redTex, "images/Rubber Ducky.png");
    ...
    while ((arrows > 0) || drawingArrow) {
        ...
        duckTimer += deltaMS;
        if (duckTimer > 5000) // going for five seconds here
        {
            CreateDuck(ducks, score, );
            duckTimer = 0;
        }
        if (deltaMS > 9) {
